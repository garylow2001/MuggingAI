...
async def get_chapters_by_course(course_id: int, db: Session = Depends(get_db)):
    ...
    chapters = (
        ...
        .filter(ChunkModel.course_id == course_id, ChunkModel.chapter_title.isnot(None))
        .group_by(ChunkModel.chapter_title)
        .all()
    )
...
async def search_chunks(
    course_id: int, query: str, limit: int = 10, db: Session = Depends(get_db)
):
...
async def get_course_stats(course_id: int, db: Session = Depends(get_db)):
    ...
    # Get basic stats
    total_chunks = (
        db.query(func.count(ChunkModel.id))
        .filter(ChunkModel.course_id == course_id)
        .scalar()
    )

    unique_chapters = (
        db.query(func.count(func.distinct(ChunkModel.chapter_title)))
        .filter(ChunkModel.course_id == course_id, ChunkModel.chapter_title.isnot(None))
        .scalar()
    )
    ...
    chunks = (
        db.query(ChunkModel.content).filter(ChunkModel.course_id == course_id).all()
    )
    ...
    return {
        ...
        "average_chunk_size": (
            round(total_words / total_chunks, 2) if total_chunks > 0 else 0
        ),
    }
