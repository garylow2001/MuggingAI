...
class RAGRetriever:
    ...
    async def retrieve_for_query(self, 
                              query: str, 
        ...
        """
        logger.info(f"Retrieving context for query: '{query}' (course_id={course_id}, limit={limit})")
        
        # Search the vector store with the query
        search_results = await self.vector_store.search(query, course_id, limit=limit*2)
    ...
    async def retrieve_multi_query(self, 
                                ...
                                total_limit: int = 8) -> List[Dict[str, Any]]:
        """
        ...
        # Gather results from all queries in parallel
        tasks = [self.retrieve_for_query(q, course_id, limit=limit_per_query) 
                for q in queries]
    ...
    async def retrieve_by_chapter(self, 
                               course_id: int,
        ...
        """
        logger.info(f"Retrieving context chunks for course {course_id}, " +
                   f"chapter {'all chapters' if chapter_title is None else chapter_title}")
        
        # Get chunks directly from vector store
        chunks = await self.vector_store.get_chunks_by_course_chapter(
            course_id, chapter_title, limit)
    ...
    def extract_keywords(self, text: str, max_keywords: int = 10) -> List[str]:
        ...
        # Filter common words
        stopwords = {'the', 'and', 'that', 'this', 'with', 'for', 'from', 'you',
                    'are', 'but', 'not', 'have', 'what', 'can', 'all', 'will'}
    ...
    async def hybrid_search(self, 
        ...
        results = await self.retrieve_multi_query(
            query_variations, course_id, limit_per_query=limit, total_limit=limit)
