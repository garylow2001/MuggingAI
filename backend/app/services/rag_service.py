...
class RAGService:
    ...
    async def generate(self, 
        ...
        """
        logger.info(f"RAG generation for query: '{query}' (course_id={course_id})")
        
        # Retrieve relevant context
        if use_hybrid_search:
            retrieved_chunks = await self.retriever.hybrid_search(
                query, course_id, limit=context_chunks)
        else:
