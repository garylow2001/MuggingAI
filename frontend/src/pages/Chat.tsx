...
import { HumanMessageBubble } from "@/components/chat/HumanMessageBubble";
...
export function Chat() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiMessage: RAGResponseMessage = {
        ...
        >
          ...
          {messages.map((message) => {
            if (message.role === "assistant") {
              // Check if it's a RAG response message (has messageType "rag")
              if ((message as BotMessage).messageType === "rag") {
                return (
                  <BotMessageBubble
                    key={message.id}
                    message={message as RAGResponseMessage}
                  />
                );
              } else {
                // Standard bot message without sources
                return (
                  <BotMessageBubble
                    key={message.id}
              ...
              return (
                <HumanMessageBubble
                  key={message.id}
                  message={message as HumanMessage}
                />
              );
            }
          })}
