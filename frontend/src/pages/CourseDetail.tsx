...
export function CourseDetail() {
  ...
  // normalize id as number for SWR keys
  const courseIdNum = courseId ? parseInt(courseId) : undefined;

  const { data: course, error: courseError } = useSWR(
    courseIdNum ? ["course", courseIdNum] : null,
    () => api.getCourse(courseIdNum!)
  );

  const { data: files = [], error: filesError } = useSWR(
    courseIdNum ? ["courseFiles", courseIdNum] : null,
    () => api.getCourseFiles(courseIdNum!)
  );

  const { data: notes = [], error: notesError } = useSWR(
    courseIdNum ? ["courseNotes", courseIdNum] : null,
    () => api.getCourseNotes(courseIdNum!)
  );

  const isLoading = !course && !courseError;

  useEffect(() => {
    if (courseError || filesError || notesError) {
      setError(
        (courseError || filesError || notesError)?.message ||
          "Failed to load course data"
      );
    } else {
      setError("");
    }
  }, [courseError, filesError, notesError]);

  const handleFileDelete = async (fileId: number) => {
    if (
      !confirm(
        "Are you sure you want to delete this file? This will also remove all associated notes and chunks."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (courseIdNum) {
        mutate(["courseFiles", courseIdNum]);
        mutate(["course", courseIdNum]);
      }
    } catch (err) {
  ...
  const handleNotesUpdate = () => {
    if (courseIdNum) mutate(["courseNotes", courseIdNum]);
  };
    ...
    <div className="container mx-auto px-4">
      ...
      >
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {notes.reduce(
                    (total, topic) => total + topic.notes.length,
                    0
                  )}
                </div>
                <p className="text-xs text-muted-foreground">
        ...
        <TabsContent value="files" className="space-y-6">
          ...
          {/* File Upload */}
          <FileUploadComponent courseId={course.id} />
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                            ...
                            <span>•</span>
                            <span>
                              {file.chunks_count} chunk
                              {file.chunks_count !== 1 ? "s" : ""}
                            </span>
                            <span>•</span>
                            <span>
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-muted-foreground mb-4">
                  Upload your first course material to get started with
                  AI-powered learning.
                </p>
          ...
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-muted-foreground">
                Interactive study features, quizzes, and spaced repetition will
                be available here.
              </p>
            </CardContent>
          </Card>
